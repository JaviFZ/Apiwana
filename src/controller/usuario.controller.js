...
const getOpinion = (request, response) => {
    let sql = "SELECT nombre , foto , opinion, opiniones.puntuacion FROM railway.opiniones JOIN railway.usuarios ON (railway.opiniones.id_pasajero=railway.usuarios.id_usuario) WHERE id_conductor=" + request.query.id_usuario;
      console.log(sql);  
      connection.query(sql, function (err, result) {
          if (err) 
              console.log(err);
